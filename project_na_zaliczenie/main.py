...
class interfejs():
    ...
    def button_znalesc(self):
        min_cena = min(self.entry_cena_od.get())
        max_cena = max(self.entry_cena_do.get())
        brend = min(self.entry_brend.get())
        typ = self.entry_typ.get()

        lista = self.typ.znalesc(min_cena, max_cena, brend, typ)
